...
func New() *SuperAgent {
	...
	debug := os.Getenv("request_DEBUG") == "1"
	...
	// disable keep alives by default, see this issue https://github.com/parnurzeal/request/issues/75
...
//
//      request.New().TLSClientConfig(&tls.Config{ InsecureSkipVerify: true}).
...
// Another example is using Golang proxy setting. This is normal prefer way to do but too verbase compared to request's Proxy:
//
//      request.New().Proxy("http://myproxy:9999").
...
//        End()
//
// While if you use at least one of querystring, request understands and automatically set the Content-Type to `application/x-www-form-urlencoded`
//
//      request.New().
//        Post("/search").
//        Send("query=tonkatsu").
...
func changeMapToURLValues(data map[string]interface{}) url.Values {
	...
	for k, v := range data {
		...
		// json.Number used to protect against a wrong (for request) default conversion
...
// For example:
//
//    resp, body, errs := request.New().Get("http://www.google.com").End()
//    if errs != nil {
//      fmt.Println(errs)
//    }
...
// This extends the flexibility and makes request fun and clean! You can use request in whatever style you love!
//
// For example:
//
//    func printBody(resp request.Response, body string, errs []error){
//      fmt.Println(resp.Status)
//    }
//    request.New().Get("http://www..google.com").End(printBody)
//
...
func (s *SuperAgent) MakeRequest() (*http.Request, error) {
	...
	//
	//     https://github.com/parnurzeal/request/pull/136
	//
	switch s.TargetType {
	case TypeJSON:
	...
	// https://github.com/parnurzeal/request/issues/164
